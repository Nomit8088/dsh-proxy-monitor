...
import { PROXY_MONITOR_ENDPOINTS, PROXY_MONITOR_ROUTE, } from './contract.js';
...
import { Config as WorkBuddyConfigSchema } from './workbuddy/index.js';
...
export function apply(ctx, config) {
    ...
    ctx.inject(['settings'], settingsCtx => {
        settingsCtx.effect(() => settingsCtx.settings.configure({ auto: false }, ctx.fiber), 'dsh-proxy-monitor: settings presentation');
    });
    /**
     ...
     */
    const handleEndpoint = async (endpoint, payload, signal) => {
        try {
    ...
    ctx.inject(['connection'], connectionCtx => {
        connectionCtx.effect(() => {
            const disposers = PROXY_MONITOR_ENDPOINTS.map(endpoint => connectionCtx.connection.fetch.register({
                ...
                fetch: async (request) => {
                    ...
                    return respond(await handleEndpoint(endpoint, payload, request.signal));
                },
            }));
            return () => {
    ...
    ctx.logger.info('dsh-proxy-monitor: %s providers, refresh every %ss', String(config.providers.get().length), String(config.refreshSeconds.get()));
}
