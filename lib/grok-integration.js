...
function liveCatalogModels(catalog) {
...
export function setupGrok(ctx, grokService) {
    ...
    ctx.inject(['llm'], llmCtx => {
        const adapter = new GrokAuthAdapter(ctx, {
            ...
            visibleModelIds: () => {
                ...
                if (snap.catalogModels.length === 0 && snap.enabledModelIds.length === 0)
                    return undefined;
                return snap.enabledModelIds;
            },
            imageModelIds: () => modelSettings.snapshot().imageModelIds,
        });
        installOrTakeOverAdapter(llmCtx.llm, GROK_ROUTE, adapter, ctx.logger);
    });
...
//# sourceMappingURL=grok-integration.js.map