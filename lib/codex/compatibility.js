...
function nodeStatus(value) {
    ...
    if (major === 22)
        return minor > 19 || (minor === 19 && patch >= 0) ? 'compatible' : 'incompatible';
...
function aggregateStatus(entries) {
    if (entries.some(entry => entry.status === 'incompatible'))
        return 'incompatible';
...
export async function detectCompatibility(options = {}) {
    ...
    const resolvedPackages = packageVersions === undefined
        ? Object.fromEntries(await Promise.all(COMPATIBILITY_PACKAGES.map(async (name) => [name, await readVersion(name)])))
        : packageVersions;
