...
import { writeWorkspaceBytes } from './binary-fs.js';
import { assertImageCapable } from './image-capability.js';
import { imageMediaType } from './read-image-enhancement.js';
...
function defaultOutputPath(now = new Date(), id = randomUUID()) {
...
function collectImageRefs(content, output) {
    for (const block of content) {
        if (block.type === 'image')
            output.push(block.attachment);
        else if (block.type === 'tool-result')
            collectImageRefs(block.content, output);
    }
}
...
async function workspaceImages(ctx, exec, paths) {
    ...
    for (const path of paths) {
        if (path.trim().length === 0)
            throw new Error('referenced_image_paths must not contain an empty path');
        const target = await ctx.fs.resolve(path, { ...cwd === undefined ? {} : { cwd }, signal: exec.signal });
        const info = await ctx.fs.stat(target, exec.signal);
        if (info === undefined)
            throw new Error(`referenced image does not exist: ${path}`);
        if (info.type !== 'file')
            throw new Error(`referenced image is not a regular file: ${path}`);
        const data = await ctx.fs.readBytes(target, exec.signal, maxBytes);
        const mediaType = imageMediaType(data);
        if (mediaType === undefined)
            throw new Error(`referenced image is not PNG, JPEG, WebP, or GIF: ${path}`);
...
function parseArgs(args) {
    ...
    if (prompt.length === 0)
        throw new Error('imagegen prompt must not be empty');
...
export function imagegenTool(ctx, credentials, policy, requestFetch = globalThis.fetch) {
    ...
    return defineTool({
        ...
        async execute(rawArgs, exec) {
            ...
            if (mediaType !== 'image/png')
                throw new Error('OpenAI Codex image response was not a PNG');
