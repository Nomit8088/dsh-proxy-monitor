 ...
 */
export function isFastModeSessionId(value) {
...
export class FastModeRegistry {
    maxSessions;
    enabledSessions = new Map();
    constructor(maxSessions = OPENAI_CODEX_FAST_MODE_MAX_SESSIONS) {
    ...
    isEnabled(sessionId) {
        if (!isFastModeSessionId(sessionId))
            return false;
    ...
    set(sessionId, enabled) {
        if (!isFastModeSessionId(sessionId))
            throw new TypeError('Invalid Fast Mode session id');
        if (typeof enabled !== 'boolean')
            throw new TypeError('Fast Mode enabled must be boolean');
        if (!enabled) {
            this.enabledSessions.delete(sessionId);
            return;
        }
