...
async function assertOwnerOnly(filename) {
    ...
    if (!metadata.isFile())
        throw new Error(`openai-codex: ${filename} is not a regular file`);
    ...
    if ((metadata.mode & 0o077) !== 0) {
        throw new Error(`openai-codex: ${filename} is readable beyond its owner (mode ${(metadata.mode & 0o777).toString(8)});`
...
function parseDocument(text, filename) {
    ...
    if (!Array.isArray(rawOrigins))
        throw new Error(`openai-codex: ${filename} origins must be an array`);
    const origins = new Set();
    for (const rawOrigin of rawOrigins) {
        if (typeof rawOrigin !== 'string')
            throw new Error(`openai-codex: ${filename} origins must contain strings`);
        try {
 ...
 */
export function normalizeTrustedOrigin(rawOrigin) {
    ...
    if (origin.username !== '' || origin.password !== '')
        throw new Error('trusted origin must not contain credentials');
    if (origin.pathname !== '/' || origin.search !== '' || origin.hash !== '') {
        throw new Error('trusted origin must not contain a path, query, or fragment');
    }
    if (origin.hostname === '' || origin.hostname.includes('*'))
        throw new Error('trusted origin host must be exact');
    ...
    if (origin.origin === 'null')
        throw new Error('trusted origin must have an HTTP(S) host');
    return origin.origin;
}
/** Resolve the sidecar path under one DSH home. */
export function openAICodexTrustedOriginsPath(dshHome) {
...
export class OpenAICodexTrustedOriginsStore {
    ...
    async trust(origin) {
        ...
        return withFileLock(this.filename, async () => {
            ...
            if (current.origins.includes(normalized))
