...
export async function collectBoundedBytes(body, declaredLength, maxBytes, signal) {
...
async function requestPinned(url, address, maxBytes, signal) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const request = requester(url, {
            ...
            void collectBoundedBytes(incoming, headerValue(incoming, 'content-length'), maxBytes, signal).then(data => { finish({ ok: true, value: { status, data } }); }, error => {
...
export const NODE_PUBLIC_HTTP_RUNTIME = {
    resolve: resolveHost,
    get: requestPinned,
};
/** Fetch bytes from a public HTTP(S) target, revalidating and repinning each redirect. */
export async function fetchPublicHttpResource(source, maxBytes, signal, runtime = NODE_PUBLIC_HTTP_RUNTIME) {
    ...
    for (let redirects = 0;; redirects += 1) {
        ...
        if (hop.data === undefined)
            throw new Error('remote image response did not contain a body');
