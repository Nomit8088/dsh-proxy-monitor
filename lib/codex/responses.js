...
export function expandNativeCompactionMarkers(input) {
    const expanded = [];
...
async function compactResponse(response, retained) {
    if (response.body === null)
        throw new Error('OpenAI Codex compact response had no body');
    ...
    const consumeEvent = (raw) => {
        ...
        if (event['type'] === 'response.output_item.done') {
            ...
            if (isRecord(item) && item['type'] === 'compaction') {
                if (compaction !== undefined)
                    throw new Error('OpenAI Codex compact response contained multiple compaction items');
                compaction = item;
            }
            return;
        }
        ...
        if (!isRecord(terminal))
            throw new Error('OpenAI Codex compact stream returned a malformed terminal event');
        const id = terminal['id'];
        if (id !== undefined && typeof id !== 'string')
            throw new Error('OpenAI Codex returned a malformed compact response id');
        responseId = id;
        const rawUsage = terminal['usage'];
        if (rawUsage !== undefined && !isRecord(rawUsage))
            throw new Error('OpenAI Codex returned malformed compact usage');
        usage = rawUsage;
        completed = true;
    };
    try {
    ...
    if (!completed)
        throw new Error('OpenAI Codex compact stream ended before response.completed');
    if (compaction === undefined)
        throw new Error('OpenAI Codex compact response did not contain a compaction item');
    return {
...
export class OpenAICodexResponseRuntime {
    ...
    enterCompaction(sessionId) {
        ...
        return () => {
            ...
            if (remaining <= 0)
                this.compactionCalls.delete(key);
            else
                this.compactionCalls.set(key, remaining);
        };
    }
    ...
    standardStream(provider, model, context, options, reuseWebSocketContext) {
        return provider.streamSimple(model, context, {
            ...
            onPayload: async (payload, payloadModel) => {
                if (!isRecord(payload))
                    throw new Error('OpenAI Codex generated a non-object Responses payload');
    ...
    nativeCompactionStream(provider, model, context, options) {
        ...
        void this.requestNativeCompaction(model, context, options).then(response => {
            const source = markerStream(model, response);
            void (async () => { for await (const event of source)
                target.push(event); })();
        }, error => {
    ...
    async requestNativeCompaction(model, context, options) {
        ...
        if (access === undefined || access.length === 0)
            throw new Error('OpenAI Codex compact request has no OAuth token');
        const messages = context.messages.length === 0 ? [] : context.messages.slice(0, -1);
        const input = expandNativeCompactionMarkers(convertResponsesMessages(model, { ...context, messages }, CODEX_TOOL_CALL_PROVIDERS, { includeSystemPrompt: false }));
        ...
        if (!isRecord(body))
            throw new Error('OpenAI Codex generated a non-object compact Responses payload');
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            ...
            if (response.ok)
                return await compactResponse(response, retained);
