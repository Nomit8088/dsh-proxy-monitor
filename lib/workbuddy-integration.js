 ...
 */
import { apply as applyWorkBuddy, WORKBUDDY_PROVIDER, WorkBuddyCredentialStore, CN_VARIANT, } from './workbuddy/index.js';
...
export function setupWorkBuddy(ctx, live) {
    ...
    try {
        ...
        const started = applyWorkBuddy(ctx, runtimeConfig);
        if (typeof started?.repoint === 'function')
            repoint = started.repoint;
    ...
    ctx.inject(['llm'], llmCtx => {
        let unwrap = () => { };
        const overlay = () => {
            unwrap = wrapAdapterCatalog(llmCtx.llm, WORKBUDDY_PROVIDER, {
