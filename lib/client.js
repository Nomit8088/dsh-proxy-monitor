window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const PROXY_MONITOR_CHANNEL = "/proxy-monitor";
		/** Durable settings namespace registered by the Host and read by the browser. */
		const PROXY_MONITOR_NAMESPACE = "dsh-proxy-monitor";
		//#endregion
		...
		*/
		function createQuotaBroker(rpc) {
			return {
				async snapshot(signal) {
					return unwrap(await rpc.call(PROXY_MONITOR_CHANNEL, "snapshot", {}, signal));
				},
				async refresh(signal) {
					return unwrap(await rpc.call(PROXY_MONITOR_CHANNEL, "refresh", {}, signal));
				},
				async accounts(signal) {
					return unwrap(await rpc.call(PROXY_MONITOR_CHANNEL, "accounts", {}, signal));
				},
				async login(id, signal) {
					return unwrap(await rpc.call(PROXY_MONITOR_CHANNEL, "login", { id }, signal));
				},
				async pollLogin(id, ticketId, signal) {
					return unwrap(await rpc.call(PROXY_MONITOR_CHANNEL, "loginPoll", {
						id,
						ticketId
					}, signal));
				},
				async logout(id, signal) {
					return unwrap(await rpc.call(PROXY_MONITOR_CHANNEL, "logout", { id }, signal)).ok;
				}
			};
		}
		//#endregion
		...
		*/
		function apply(ctx) {
			...
			const scope = context.settingsScope.bind({ namespace: PROXY_MONITOR_NAMESPACE });
