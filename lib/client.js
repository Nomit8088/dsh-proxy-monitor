window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function LoginInstruction({ method }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
				...
				children: [
					...
					" ",
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("a", {
						className: AccountBlock_module_css_default.linkButton,
						href: method.url,
						target: "_blank",
						rel: "noreferrer",
						children: "打开授权页"
		...
		*/
		...
		function refreshReason(message) {
			return /verify your account to continue/i.test(message) ? `Google 拒绝了插件本次请求并返回验证提示；若官方 Antigravity 仍可读取额度，不能据此断定账号整体未验证。请检查请求端点与该插件的 OAuth 授权。${message}` : message;
		}
				...
				try {
					if (refreshLive) try {
						...
						if (quota.models !== void 0) {
							...
							if (quota.catalogError) setError(`活体模型目录刷新失败（仅显示上次保存的列表，如有）：${refreshReason(quota.catalogError)}`);
							else if (quota.quotaError) setError(`模型目录已刷新，但 5 小时/每周分组额度读取失败；如有模型剩余额度，仅来自模型接口：${refreshReason(quota.quotaError)}`);
							return;
						}
					...
					if (refreshFailure) setError(`活体刷新失败，显示上次保存的目录；不能据此认定模型仍可用：${refreshReason(refreshFailure)}`);
				} catch (caught) {
