	...
	factory: (require) => {
		...
		*/
		...
		function refreshReason(message) {
			return /verify your account to continue/i.test(message) ? `Google 要求此账号先完成验证；OAuth 登录成功只证明有凭据，不代表已获 Cloud Code Assist 额度/模型权限。请按 Google 的提示验证账号后再刷新。${message}` : message;
		}
				...
				try {
					if (refreshLive) try {
						...
						if (quota.models !== void 0) {
							...
							if (quota.catalogError) setError(`额度已读取，但活体模型目录刷新失败（仅显示上次保存的列表，如有）：${refreshReason(quota.catalogError)}`);
							return;
						}
