...
function kindOfSeconds(seconds) {
...
function labelOfSeconds(seconds) {
    if (seconds === undefined)
        return 'Limit';
    if (seconds <= 6 * 60 * 60)
...
function spanOfSeconds(seconds) {
    if (seconds === undefined)
        return undefined;
    const hours = seconds / 3600;
    if (hours < 24)
        return `${String(Math.round(hours))}h window`;
    ...
    for (const group of rateLimits) {
        ...
        for (const [index, entry] of groupWindows.entries()) {
            ...
            const label = groupWindows.length > 1 || groupName === undefined
                ...
                : groupName;
            windows.push(windowOf({ id: `${groupId}-${String(index)}`, label, kind }, {
 ...
 */
export async function readCodex(ctx, pluginBase) {
    const base = { id: 'codex', name: 'Codex', fetchedAt: Date.now() };
    try {
        ...
        if (isRecord(individual)) {
            ...
            if (remaining !== undefined) {
                windows.push(windowOf({ id: 'individual', label: 'Individual limit', kind: 'other' }, {
                    usedPercent: usedFromRemaining(clampPercent(remaining)),
