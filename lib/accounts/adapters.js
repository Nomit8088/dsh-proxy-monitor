 ...
 */
export function accountFromQuota(id, name, quota, login = LOGIN_PENDING, canLogout = false, canReauth = false) {
    if (quota.status === 'ok') {
        return {
            id,
            name,
 ...
 * @returns an account adapter for that provider.
 */
function snapshotBackedAdapter(id, name, readSnapshot) {
 ...
 */
export function createQuotaBackedAdapters(readSnapshot) {
    return [
