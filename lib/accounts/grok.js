 ...
 * @returns a quota row.
 */
function quotaFromUsage(usage, signedIn) {
    const base = { id: ID, name: NAME, fetchedAt: Date.now() };
    ...
    const window = windowOf({ id: 'weekly-credits', label: 'Weekly credits', kind: 'weekly' }, {
 ...
 */
export function createGrokQuotaReader(service) {
    return async () => {
        try {
 ...
 */
export class GrokAccountAdapter {
    service;
    readSnapshot;
    id = ID;
     ...
     */
    constructor(service, readSnapshot) {
