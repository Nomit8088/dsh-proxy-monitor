...
async function fetchAvailableModelsFromEndpoint(endpoint, token, projectId) {
  try {
    ...
    if (!response.ok) {
      ...
      return { endpoint, status: response.status, error: safeError(message).slice(0, 160) };
    }
    if (!isRecord(data) || !isRecord(data.models)) {
      return { endpoint, status: response.status, error: "malformed available-models response" };
    }
    return { endpoint, status: response.status, data };
  } catch (error) {
    setLastError(error);
    return { endpoint, error: safeError(error).slice(0, 160) };
  }
}
...
async function fetchMergedAvailableModels(token, projectId) {
  ...
  if (!lastEndpoint || (Object.keys(mergedModels).length === 0 && failures.length > 0)) {
    // An empty 200 from one endpoint plus a 403 from the other is not proof
    // that Google offers no models. Keep the saved catalog rather than replace
    // it with an empty projection of an incomplete read.
    throw new Error(`Antigravity model discovery ${lastEndpoint ? "incomplete" : "failed"}: ${failures.join("; ") || "no endpoint available"}`.slice(0, 400));
  }
  return {
...
async function fetchMergedQuotaSummary(token) {
  ...
  if (!best) {
    ...
    // Do not make a second, production-only request with different headers and
    // lose the two endpoint statuses: a Google account-verification 403 is an
    // entitlement failure, not a reason to retry until it becomes a generic
    // "quota unavailable" message.
    throw new Error(`Antigravity quota summary failed: ${failures.join("; ")}`.slice(0, 400));
  }
  return best;
}
...
/**
 * Build the live catalog from an available-models payload.
 *
 * Hardcoded {@link MODELS} are shape templates and routing aliases only:
 * a family that is not in the live payload is omitted rather than injected.
 * Unmatched live models (new releases) are appended as-is.
 */
export function parseCatalogModels(data) {
  const raw = isRecord(data) ? data : {};
  const modelsObj = isRecord(raw.models) ? raw.models : {};
  const rawModels = [];
  ...
  for (const live of rawModels) {
    if (!matchedRawIds.has(live.id)) consolidated.push(live);
  }
...
/** Models whose inferred catalog entry already declares image input. */
export function defaultImageModelIdsFrom(models) {
 ...
 *
 * DSH intercepts attached images (`projectImagesForTextModel`) when
 * `inputModalities` is defined and does not include `"image"`. Selected
 * models must therefore declare image input so native image parts pass through.
 */
export function withUserImageSupport(model, imageIds) {
  const supportsImages = imageIds instanceof Set ? imageIds.has(model.id) : Array.isArray(imageIds) && imageIds.includes(model.id);
  return {
    ...model,
 ...
 * on/off choices and auto-enable newly discovered ids so a dynamic catalog
 * actually surfaces new models.
 */
export function mergeEnabledModelIds(current, catalogModels) {
 ...
 * the inferred default from the live catalog (gemini/claude → image).
 */
export function mergeImageModelIds(current, catalogModels) {
  const previousIds = new Set((current?.catalogModels || []).map((model) => model.id));
  const previousImage = new Set(
    Array.isArray(current?.imageModelIds)
      ? current.imageModelIds
      : defaultImageModelIdsFrom((current?.catalogModels || []).length ? current.catalogModels : MODELS),
  );
...
export async function fetchAccountQuota(store = new FileCredentialStore(), modelSettings) {
  ...
  let catalogError;
  try {
    available = await fetchMergedAvailableModels(token, projectId);
  } catch (error) {
  ...
  if (modelSettings && available) {
    const current = await modelSettings.read();
    const imageModelIds = mergeImageModelIds(current, catalogModels);
    await modelSettings.setCatalogModels(
      catalogModels.map((model) => withUserImageSupport(model, imageModelIds)),
      {
...
function normalizeModelSettings(raw) {
  // An account without a successful live catalog has no proven selectable
  // models. Static templates (including 3.7 Flash) must not be advertised as
  // available just because this account's discovery failed or returned 403.
  const enabledModelIds = normalizeIdList(raw?.enabledModelIds) ?? [];
  return {
    enabledModelIds,
    imageModelIds: normalizeIdList(raw?.imageModelIds),
...
export class FileModelSettingsStore {
  ...
  async write(settings) {
    ...
    const payload = {
      ...
      updatedAt: Date.now(),
      ...(normalized.imageModelIds === undefined ? {} : { imageModelIds: normalized.imageModelIds }),
    };
    await mkdir(dirname(this.#file), { recursive: true });
    const tempFile = `${this.#file}.tmp`;
  ...
  setImageModelIds(imageModelIds) {
    return this.modify((current) => ({ ...current, imageModelIds }));
  }
  ...
  setCatalogModels(catalogModels, options = {}) {
    return this.modify((current) => ({
      ...
      ...(Array.isArray(options.imageModelIds) ? { imageModelIds: options.imageModelIds } : {}),
    }));
  }
}
...
function catalogModelsOf(settings) {
  // MODELS is only a family/routing template. Neither a never-fetched account
  // nor a successful *empty* live catalog may resurrect that static table as
  // an assertion that Google offers those models to this credential.
  return normalizeModelSettings(settings).catalogModels;
}

export function imageModelIdsOf(settings) {
  const normalized = normalizeModelSettings(settings);
  if (normalized.imageModelIds !== undefined) return [...normalized.imageModelIds];
  return defaultImageModelIdsFrom(catalogModelsOf(normalized));
}

function configuredModels(settings) {
  const enabled = enabledModelSet(settings);
  const imageIds = new Set(imageModelIdsOf(settings));
  return catalogModelsOf(settings)
    .filter((model) => enabled.has(model.id))
    .map((model) => withUserImageSupport(model, imageIds))
    .sort(compareAntigravityModels);
}

function modelFromSettings(settings, modelId) {
  const imageIds = new Set(imageModelIdsOf(settings));
  const found =
    catalogModelsOf(settings).find((model) => model.id === modelId) || modelById(modelId) || inferModelShape(modelId);
  return withUserImageSupport(found, imageIds);
}
...
function modelOptionsPayload(settings, quota) {
  ...
  const options = allModels.map((model) => {
    ...
    return {
      ...
      reasoningEfforts: applied.reasoningEfforts,
      enabled: enabled.has(applied.id),
      ...quotaInfo,
    };
  });
                ...
                if (request.method === "POST") {
                  ...
                  if (!hasEnabled && !hasImage) {
                    return sendJson(response, 400, {
                      ok: false,
                      error: "enabledModelIds or imageModelIds must be an array",
                    });
                  }
                  const settings = await modelSettings.modify((current) => {
                    const next = {
                      ...current,
                      ...(hasEnabled ? { enabledModelIds: body.enabledModelIds } : {}),
                      ...(hasImage ? { imageModelIds: body.imageModelIds } : {}),
                    };
                    const imageIds = imageModelIdsOf(next);
                    return {
                      ...next,
                      catalogModels: catalogModelsOf(next).map((model) => withUserImageSupport(model, imageIds)),
                    };
                  });
                  emitLlmAdaptersUpdated(ctx);
                  return sendJson(response, 200, {
...
export function apply(ctx, options = {}) {
  const store = options.store ?? new FileCredentialStore(credentialPath());
  const modelSettings = options.modelSettings ?? new FileModelSettingsStore(modelSettingsPath());
  if (options.registerAdapter !== false) {
    try {
      const existing =
        typeof ctx.llm?.listProviders === "function" &&
        ctx.llm.listProviders().some((provider) => provider.id === PROVIDER);
      if (!existing && ctx.llm) {
        ctx.llm.registerAdapter([PROVIDER], new AntigravityAdapter(store, modelSettings));
      }
    } catch (error) {
      setLastError(error);
    }
