...
export function catalogRowSupportsImages(model) {
 ...
 */
export function mergeImageModelIds(current, catalogModels, fallbackWhenMissing = []) {
    const previousIds = new Set((current?.catalogModels ?? []).map(model => model.id));
    const previousImage = new Set(Array.isArray(current?.imageModelIds)
        ? current.imageModelIds
        : defaultImageModelIdsFrom((current?.catalogModels ?? []).length > 0 ? current.catalogModels : fallbackWhenMissing));
    return catalogModels
        .filter(model => {
        if (!previousIds.has(model.id))
            return catalogRowSupportsImages(model);
