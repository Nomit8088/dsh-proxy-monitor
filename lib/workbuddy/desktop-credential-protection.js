...
function parseWrappedField(field, value) {
    if (typeof value !== 'object' || value === null || Array.isArray(value))
        return undefined;
    const wrapped = value;
    if (wrapped['$wbEncrypted'] !== 1 || typeof wrapped['envelope'] !== 'string')
        return undefined;
    let inner;
    try {
...
export function classifyDesktopAuthDocument(text) {
    ...
    for (const field of AUTH_FIELDS) {
        ...
        if (wrapped === undefined && value !== undefined)
            return { format: 'unrecognized' };
...
export function unwrapDesktopAuthDocument(classification, openField) {
    const rebuilt = structuredClone(classification.wrapped.document);
...
export function pickWorkBuddyExeFromRegistry(stdout) {
...
function windowsUninstallWorkBuddyExe() {
...
function windowsWorkBuddyGuesses() {
...
export function resolveWorkBuddyElectronPath() {
    const fromEnv = process.env[WORKBUDDY_ELECTRON_BIN_ENV]?.trim();
    if (fromEnv !== undefined && fromEnv !== '' && fileExists(fromEnv))
        return fromEnv;
    if (process.platform === 'darwin' && fileExists(MACOS_ELECTRON_PATH))
...
export class WorkBuddyAtRestKeyProvider {
    ...
    async protectorKeyFor(requested) {
        ...
        if (cached !== undefined && requested.includes(cached.keyId))
            return cached.key;
        ...
        if (!requested.includes(resolved.keyId)) {
            throw new WorkBuddyElectronPathError('encrypted-credential-unreadable', `WorkBuddy's current at-rest key (id ${resolved.keyId}) does not match the credential's envelope (id ${requested.join(' or ')}); open the WorkBuddy app once to reseal the sign-in`);
        }
        return resolved.key;
    }
    ingest(text) {
        ...
        if (payload === undefined) {
            throw new WorkBuddyElectronPathError('encrypted-credential-unreadable', 'WorkBuddy key helper returned an unusable at-rest payload');
        }
    ...
    async spawnPayload() {
        ...
        if (electronPath === undefined || electronPath === '') {
            throw new WorkBuddyElectronPathError('electron-binary-unavailable', `WorkBuddy 5.6 encrypted the desktop sign-in; open the WorkBuddy app once, or set ${WORKBUDDY_ELECTRON_BIN_ENV} to WorkBuddy.exe`);
        }
        return await new Promise((resolve, reject) => {
            execFile(electronPath, ['-e', HELPER_SCRIPT], {
                ...
                if (error !== null && error !== undefined) {
                    ...
                    if (code === 'ENOENT') {
                        reject(new WorkBuddyElectronPathError('electron-binary-not-found', `WorkBuddy.exe was not found at ${electronPath}; open the WorkBuddy desktop app once, or set ${WORKBUDDY_ELECTRON_BIN_ENV}`));
                        return;
                    }
                    ...
                    reject(new WorkBuddyElectronPathError('encrypted-credential-unreadable', `the WorkBuddy key helper (${electronPath}) ${reason}`));
                    return;
                }
...
//# sourceMappingURL=desktop-credential-protection.js.map