import { PROXY_MONITOR_ENTRY } from "../contract.js";
...
function writeUserCatalog(next) {
	...
	writeFileSync(temp, JSON.stringify({ ...next, updatedAt: Date.now() }, null, 2), { mode: 0o600 });
	renameSync(temp, workbuddyUserCatalogPath());
	return next;
}

function applyUserImage(info) {
	const prefs = readUserCatalog();
	if (!Array.isArray(prefs.imageModelIds)) return info;
	return { ...info, supportsImages: prefs.imageModelIds.includes(info.id) };
}

function workbuddyOptionsPayload(models) {
	const prefs = readUserCatalog();
	const enabled = new Set(Array.isArray(prefs.enabledModelIds) ? prefs.enabledModelIds : models.map((model) => model.id));
	const imageIds = new Set(Array.isArray(prefs.imageModelIds) ? prefs.imageModelIds : defaultImageModelIdsFrom(models.map((model) => ({ id: model.id, supportsImages: model.supportsImages }))));
	return {
		...
		options: models.map((model) => ({
			...
			supportsImages: imageIds.has(model.id),
...
function createWorkBuddyAdapter(options) {
	...
	const buildModels = () => {
		...
		return catalog.current().map((info) => toPiModel(applyUserImage(info), baseUrl, observe?.(info.id), providerId));
	};
	const provider = {
		...createProvider({
			id: providerId,
...
var WorkBuddyPiAiAdapter = class extends PiAiAdapter {
	...
	async listModels(provider) {
		const mapped = (await super.listModels(provider)).map((model) => {
		...
		const prefs = readUserCatalog();
		if (!Array.isArray(prefs.enabledModelIds)) return mapped;
		const enabled = new Set(prefs.enabledModelIds);
		return mapped.filter((model) => enabled.has(model.id));
	}
...
function registerWorkBuddyModelsRoute(ctx, deps) {
	const path = deps.path ?? WORKBUDDY_MODELS_PATH;
	ctx.effect(() => {
		const dispose = ctx.webServer.register({
			kind: "exact",
			path,
			handler: async (req, res) => {
				try {
					...
					if (req.method === "GET") {
						json(res, 200, { ok: true, value: workbuddyOptionsPayload(deps.models()) });
						return;
					}
					if (req.method !== "POST") {
					...
					const live = deps.models().map((model) => ({
						id: model.id,
						name: model.name,
						supportsImages: model.supportsImages === true,
					}));
					if (body.refresh === true) {
						if (typeof deps.refresh === "function") await deps.refresh();
						const refreshed = deps.models().map((model) => ({
							id: model.id,
							name: model.name,
							supportsImages: model.supportsImages === true,
						}));
						const current = readUserCatalog();
						writeUserCatalog({
							catalogModels: refreshed,
							enabledModelIds: mergeEnabledModelIds(current, refreshed),
							imageModelIds: mergeImageModelIds(current, refreshed),
						});
						deps.invalidate?.();
						json(res, 200, { ok: true, value: workbuddyOptionsPayload(deps.models()) });
						return;
					...
					if (!hasEnabled && !hasImage) {
						json(res, 400, { ok: false, error: "enabledModelIds or imageModelIds must be an array" });
						return;
					}
					const current = readUserCatalog();
					writeUserCatalog({
						catalogModels: live,
						enabledModelIds: hasEnabled ? body.enabledModelIds : (current.enabledModelIds ?? live.map((model) => model.id)),
						imageModelIds: hasImage ? body.imageModelIds : (current.imageModelIds ?? defaultImageModelIdsFrom(live)),
					});
					deps.invalidate?.();
					json(res, 200, { ok: true, value: workbuddyOptionsPayload(deps.models()) });
				} catch (error) {
	...
	}, "dsh-workbuddy-connect: Web model catalog");
}
//#endregion
...
const inject = ["llm"];
/**
* Path from the host entry's config root to the WorkBuddy profile object.
*
* The 0.1.7 settings seam addresses configuration by profile entry id and knows
* no per-plugin namespaces, so the merged plugin declares these fields as
* volatile fields of its own Config, nested under `workbuddy`. The
* configurable-provider directory entry points there; the card keys the
* standalone plugin used ('workbuddy' / 'workbuddy-ai') no longer name a
* settings section.
*/
const MERGED_SETTINGS_PATH = ["workbuddy"];
/**
	...
	try {
		...
		try {
			releaseDirectory = ctx.llm.registerConfigurableProviders([{
				provider: variant.id,
				displayName: variant.displayName,
				settingsNs: PROXY_MONITOR_ENTRY,
				settingsPath: MERGED_SETTINGS_PATH,
				declared: false
			}]);
		} catch (error) {
	...
	*/
	...
	ctx.inject(["webServer"], (webCtx) => {
		for (const runtime of runtimes) {
			...
			if (runtime.variant.id === "workbuddy") {
				registerWorkBuddyModelsRoute(webCtx, {
					path: WORKBUDDY_MODELS_PATH,
					models: () => runtime.catalog.current(),
					invalidate: () => runtime.invalidate(),
					refresh: async () => {
						const credential = await runtime.store.current();
						if (credential === void 0) throw new Error("WorkBuddy is signed out");
	...
	/**
	* Re-point every variant's credential store at the configured desktop file.
	*
	* `current()` reads the host plugin's live Config through the getter-backed
	* object it passes in, so the value is always fresh without any subscription
	* here; only the stores need telling, because each captured its path when it
	* was built. The host calls this after every configuration commit — a moved
	* credential file is the one change a later read cannot pick up on its own.
	*/
	const repointStores = () => {
		const next = current();
