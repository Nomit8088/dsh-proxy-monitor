...
export const PROVIDER_ORDER = READERS.map(reader => reader.id);
/**
 ...
 */
function readersWith(overrides) {
...
export class QuotaCollector {
    ...
    setPluginBase(pluginBase) {
        if (pluginBase === this.options.pluginBase)
            return;
    ...
    setInterval(intervalMs) {
        if (intervalMs === this.options.intervalMs)
            return;
    ...
    async collect() {
        ...
        const settled = await Promise.all(readersWith(this.options.overrides).map(async (reader) => {
            try {
