 ...
 */
export function parseLiveModels(payload) {
...
export function liveModelName(id) {
    return id
        .split('-')
 ...
 */
export function synthesizeModel(template, fact) {
    return {
        ...template,
        id: fact.id,
...
export class GrokModelCatalog {
    ...
    ensureFresh() {
        if (this.inflight !== undefined || Date.now() < this.nextAttemptAt)
            return;
    ...
    async fetchOnce() {
        ...
        try {
            ...
            const facts = parseLiveModels(JSON.parse(text));
            this.facts = facts;
            ...
            this.options.warn?.(`live model listing failed (${error instanceof Error ? error.name : 'Error'}); serving the installed catalog`);
        }
    }
}
