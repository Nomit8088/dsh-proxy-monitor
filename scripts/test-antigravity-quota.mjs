...
try {
  await check('quota remains readable if both available-models endpoints return 403', async () => {
    let writes = 0
  ...
  await check('both quota endpoints denied keep their status and account-verification reason', async () => {
    globalThis.fetch = async url => {
      const path = String(url)
      if (path.includes('loadCodeAssist')) return json({ projectId: 'fixture-project' })
