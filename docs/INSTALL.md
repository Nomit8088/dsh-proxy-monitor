...
| **B. `dsh plugin` + Release tgz** | `... add https://github.com/Nomit8088/dsh-proxy-monitor/releases/download/v0.1.0/dsh-external-dsh-proxy-monitor-0.1.0.tgz` | 固定产物、git 网络不稳、内网分发 |
...
# 钉具体版本（tag / commit 都行）
dsh plugin --profile web add github:Nomit8088/dsh-proxy-monitor#v0.1.0
...
```sh
dsh plugin --profile web add ./dsh-external-dsh-proxy-monitor-0.1.0.tgz
```
