...
| **B. `dsh plugin` + Release tgz** | `... add https://github.com/Nomit8088/dsh-proxy-monitor/releases/download/v0.2.4/dsh-external-dsh-proxy-monitor-0.2.4.tgz` | 固定产物、git 网络不稳、内网分发 |
...
```sh
npm pack
gh release create v0.2.4 ./dsh-external-dsh-proxy-monitor-0.2.4.tgz --title v0.2.4 --generate-notes
```
...
```sh
dsh plugin --profile web add ./dsh-external-dsh-proxy-monitor-0.2.4.tgz
```
