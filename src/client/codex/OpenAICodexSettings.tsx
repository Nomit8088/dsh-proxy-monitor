...
import type { OpenAICodexUsage } from "../../codex/usage.js";
import type {
...
export function OpenAICodexSettings({ t }: OpenAICodexSettingsProps) {
  ...
  useEffect(() => {
    void jsonRequest<ModelCatalogSettings>(MODEL_CATALOG_PATH, "POST", {
      refresh: true,
    }).then(
      (value) => {
  ...
  const catalogOptions = (modelCatalog?.availableModels ?? []).map((model) => ({
    ...
    supportsImages:
      modelCatalog?.imageModels?.includes(model.id) ?? model.supportsImages,
  ...
  const catalogIds = () => ({
    ...
    imageModels: catalogOptions
      .filter(optionSupportsImages)
    ...
    <section style={pageStyle} aria-labelledby="openai-codex-settings-title">
      ...
      <ModelCatalogPanel
        ...
        onToggleEnabled={(modelId, enabled) => {
          ...
          void saveModelCatalog([...current], catalogIds().imageModels);
        }}
        onToggleImage={(modelId, supportsImages) => {
          const current = new Set(catalogIds().imageModels);
          if (supportsImages) current.add(modelId);
          else current.delete(modelId);
          void saveModelCatalog(catalogIds().models, [...current]);
        }}
        onSetAllEnabled={(enabled) => {
          void saveModelCatalog(
            enabled ? catalogOptions.map((option) => option.id) : [],
            catalogIds().imageModels
          );
        }}
      />
      <div style={cardStyle}>
