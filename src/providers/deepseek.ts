...
import { fetchJson, isRecord, num, reasonOf, str, windowOf, type ProviderContext } from './util.js'
 ...
 */
export async function readDeepSeek(ctx: ProviderContext): Promise<ProviderQuota> {
  const base = { id: 'deepseek' as const, name: 'DeepSeek', fetchedAt: Date.now() }
  ...
  try {
    ...
    if (!isRecord(body)) throw new Error('malformed balance response')
    ...
    for (const entry of infos) {
      ...
      const parts: string[] = []
      if (topped !== undefined && topped !== '0.00') parts.push(`topped up ${symbolOf(currency)}${topped}`)
      if (granted !== undefined && granted !== '0.00') parts.push(`granted ${symbolOf(currency)}${granted}`)
      windows.push(
        windowOf(
          { id: `balance-${currency.toLowerCase()}`, label: `${currency} balance`, kind: 'balance' },
 ...
 */
export function hasBalanceRows(value: unknown): boolean {
