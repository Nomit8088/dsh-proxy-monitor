...
const WINDOW_FIELDS: readonly { field: string; label: string; kind: WindowKind }[] = [
 ...
 */
export async function readClaude(ctx: ProviderContext): Promise<ProviderQuota> {
  const base = { id: 'claude' as const, name: 'Claude', fetchedAt: Date.now() }
  ...
  try {
    ...
    if (!isRecord(payload)) throw new Error('malformed usage response')
    ...
    for (const spec of WINDOW_FIELDS) {
      ...
      windows.push(
        windowOf(
          { id: spec.field, label: spec.label, kind: spec.kind },
          { usedPercent: clampPercent(utilization), resetAt: toIso(entry['resets_at']) },
        ),
      )
    }
