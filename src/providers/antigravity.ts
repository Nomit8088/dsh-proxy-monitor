...
const CREDENTIAL_FILE = ['storages', 'antigravity-oauth.json'] as const
 ...
 * knows how to group.
 */
function kindOfWindow(window: unknown): WindowKind {
  if (typeof window !== 'string') return 'other'
  const lowered = window.toLowerCase()
  if (lowered.includes('5h') || lowered.includes('session') || lowered.includes('five')) return 'session'
  if (lowered.includes('week')) return 'weekly'
 ...
 */
function windowsFromPlugin(payload: Record<string, unknown>): QuotaWindow[] {
 ...
 */
function headlineIndex(windows: readonly QuotaWindow[]): number | undefined {
...
async function readCredential(home: string): Promise<Record<string, unknown> | undefined> {
  try {
 ...
 */
async function readUpstream(home: string): Promise<Record<string, unknown>> {
  const credential = await readCredential(home)
  const token = credential === undefined ? undefined : str(credential, 'access')
  if (token === undefined) throw new Error('no Antigravity credential at storages/antigravity-oauth.json')
  ...
  if (!isRecord(body)) throw new Error('malformed quota response')
  return body
}
 ...
 */
function windowsFromUpstream(body: Record<string, unknown>): QuotaWindow[] {
  ...
  for (const group of groups) {
    ...
    for (const bucket of buckets) {
      ...
      windows.push(
        windowOf(
          { id: `${groupName}:${windowId}`, label: groupName, kind: kindOfWindow(bucket['window']) },
          {
            usedPercent: usedFromRemaining(clampPercent(remaining * 100)),
