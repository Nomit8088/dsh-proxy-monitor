...
/** One billing package as the vendor reports it. */
...
function accountsOf(credits: Record<string, unknown>): CreditAccount[] {
 ...
 */
export async function readWorkBuddy(ctx: ProviderContext, pluginBase: string): Promise<ProviderQuota> {
  const base = { id: 'workbuddy' as const, name: 'WorkBuddy', fetchedAt: Date.now() }
  try {
    ...
    if (totalSize > 0) {
      windows.push(
        windowOf(
          { id: 'credits', label: 'All packages', kind: 'credit' },
          {
    ...
    for (const [index, row] of accounts.entries()) {
      if (row.size === undefined || row.size <= 0 || row.remain === undefined) continue
      windows.push(
        windowOf(
          { id: `package-${String(index)}`, label: row.packageName ?? `Package ${String(index + 1)}`, kind: 'credit' },
          {
