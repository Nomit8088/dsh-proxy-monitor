...
function selectEntry(file: Record<string, unknown>): Record<string, unknown> | undefined {
  const entries = Object.entries(file).filter((pair): pair is [string, Record<string, unknown>] =>
    isRecord(pair[1]),
  )
  if (entries.length === 0) return undefined
  const exact =
    entries.find(([key]) => key.endsWith(`::${GROK_CLIENT_ID}`)) ??
...
function accessTokenOf(entry: Record<string, unknown>): string | undefined {
 ...
 */
export async function readGrok(ctx: ProviderContext): Promise<ProviderQuota> {
  const base = { id: 'grok' as const, name: 'Grok', fetchedAt: Date.now() }
  ...
  try {
    ...
    const payload = await fetchJson(USAGE_ENDPOINT, {
      ...
      headers: {
        ...
        'user-agent': 'dsh-proxy-monitor/0.2.2',
      },
    })
