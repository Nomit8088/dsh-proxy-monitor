...
function loopbackBase(ctx: Context): string {
...
interface WorkBuddyStatusModel {
  id: string
  name?: string
...
interface WorkBuddyPrefs {
  enabledModelIds: string[]
  imageModelIds: string[]
  catalogModels: Array<{ id: string; name: string; supportsImages: boolean }>
  updatedAt: number
}
...
async function readPrefs(): Promise<WorkBuddyPrefs> {
  try {
    const parsed = JSON.parse(await readFile(prefsPath(), 'utf8')) as Partial<WorkBuddyPrefs>
    return {
...
async function writePrefs(next: WorkBuddyPrefs): Promise<WorkBuddyPrefs> {
...
function inferImage(id: string): boolean {
  return /(?:^|[-_])(?:vl|vision|5v|4v|image)/i.test(id)
}

function optionsFromStatus(models: WorkBuddyStatusModel[], prefs: WorkBuddyPrefs) {
  ...
  const enabled = new Set(
    prefs.enabledModelIds.length > 0 || prefs.catalogModels.length > 0
      ? prefs.enabledModelIds
      : catalog.map(model => model.id),
  )
  const imageIds = new Set(
    prefs.imageModelIds.length > 0 || prefs.catalogModels.length > 0
      ? prefs.imageModelIds
...
async function loadWorkBuddyStatus(ctx: Context): Promise<{
  models: WorkBuddyStatusModel[]
  probeKey?: string
}> {
  ...
  const models = Array.isArray(body['models'])
    ? body['models'].flatMap((row): WorkBuddyStatusModel[] => {
...
async function refreshWorkBuddyCatalog(ctx: Context, probeKey: string): Promise<void> {
...
export async function overlayWorkBuddyAdapterModels<T extends { id: string; inputModalities?: readonly string[] }>(
  models: readonly T[],
): Promise<T[]> {
          ...
          handler: async (request, response) => {
            try {
              ...
              if (body['refresh'] === true) {
                ...
                try { ctx.emit('llm/adapters-updated') } catch { /* picker refresh */ }
              ...
              const prefs = await writePrefs({
                ...
                enabledModelIds: hasEnabled ? (body['enabledModelIds'] as string[]) : current.enabledModelIds,
                imageModelIds: hasImage ? (body['imageModelIds'] as string[]) : current.imageModelIds,
                updatedAt: Date.now(),
              })
              try { ctx.emit('llm/adapters-updated') } catch { /* picker refresh */ }
              return sendJson(response, 200, { ok: true, value: optionsFromStatus(status.models, prefs) })
