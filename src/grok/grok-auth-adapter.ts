...
import type { AuthContext, CredentialStore, Provider } from '@earendil-works/pi-ai'
 ...
 */
export function grokAuthInjection(): { credentials: CredentialStore; authContext: AuthContext } {
  const credentials: CredentialStore = {
    read: async () => undefined,
    list: async () => [],
    modify: async () => {
      throw new LlmError(
        'llm-grok-auth: pi-ai credential persistence is disabled; use the Grok auth coordinator',
...
function grokProvider(
  displayName: string,
  baseUrl: string,
  catalog: GrokModelCatalog | undefined,
  imageModelIds?: () => readonly string[] | undefined,
): Provider {
...
export class GrokAuthAdapter extends PiAiAdapter {
  private readonly visibleModelIds: (() => readonly string[] | undefined) | undefined
  ...
  constructor(ctx: Context, options: GrokAuthAdapterOptions) {
    ...
    const profiles: ReadonlyMap<string, ResolvedPiAiProviderProfile> = new Map([[GROK_ROUTE, profile]])
    super({
      profiles: () => profiles,
