    ...
    getModels() {
      ...
      if (template !== undefined) {
        ...
        if (!models.some((model) => model.id === GPT_6_LUNA)) {
          const lunaTemplate =
            models.find((model) => model.id === "gpt-5.6-luna") ?? template;
          models.push({
            ...lunaTemplate,
            id: GPT_6_LUNA,
            name: "GPT-6 Luna",
...
function withOpenAICodexLiveModels(
  provider: Provider,
  live?: CodexLiveModelCatalog
): Provider {
  if (live === undefined) return provider;
  const getModels = provider.getModels;
  return {
    ...provider,
    getModels() {
 ...
 */
export function withOpenAICodexImageModalities(
  provider: Provider,
  imageModelIds?: () => readonly string[] | undefined
): Provider {
  if (imageModelIds === undefined) return provider;
  const getModels = provider.getModels;
  return {
    ...provider,
    getModels() {
      const ids = imageModelIds();
      if (ids === undefined) return getModels.call(provider);
...
export function createOpenAICodexAdapter(
  ...
  const profiles = (): Map<string, ResolvedPiAiProviderProfile> => {
    ...
    const configuredProvider = withOpenAICodexImageModalities(
      withOpenAICodexContextWindow(
        provider,
        nextContextWindow,
