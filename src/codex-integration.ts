...
} from "./codex/search.js";
import type { OpenAICodexSearchRequestRecord } from "./codex/search.js";
...
export function setupCodex(
  ctx: Context,
  credentialsStore?: OpenAICodexCredentialStore
): CodexRuntimeComponents {
  installOpenAICodexSearchEvent();
  ...
  ctx.inject(["llm"], (llmCtx) => {
    const adapter = createOpenAICodexAdapter(
      ...
      () => imageTools.modelCatalogSnapshot().imageModels,
      liveCatalog
    );
    installOrTakeOverAdapter(llmCtx.llm as never, OPENAI_CODEX_PROVIDER, adapter, ctx.logger);
  });
  ...
  ctx.inject(["web"], (webCtx) => {
    webCtx.web.registerSearchProvider(
      new OpenAICodexSearchProvider({
        ...
        resolveRequestId: () =>
          String(ctx.get("agents")?.currentInitiator()?.session.id ?? randomUUID()),
        recordRequest: (request: OpenAICodexSearchRequestRecord) => {
  ...
  ctx.inject(["webServer"], (webCtx) => {
    try {
      ...
      ctx.logger.warn(
        "dsh-proxy-monitor: openai-codex auth routes already claimed: %s",
        String(error)
      );
    }
  });
        ...
        webCtx.webServer.register({
          ...
          handler: async (req, res) => {
            ...
            try {
              ...
              const imageModels = Array.isArray(body["imageModels"]) ? body["imageModels"] as string[] : undefined;
