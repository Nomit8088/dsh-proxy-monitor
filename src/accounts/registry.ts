...
export class AccountRegistry {
  private readonly adapters = new Map<ProxiedProviderId, AccountAdapter>()
  ...
  async accounts(): Promise<ProviderAccount[]> {
    const rows = await Promise.all(
      [...this.adapters.values()].map(async (adapter): Promise<ProviderAccount> => {
        try {
   ...
   */
  async beginLogin(id: ProxiedProviderId): Promise<LoginTicket | undefined> {
    const adapter = this.adapters.get(id)
    if (adapter === undefined || !canLogin(adapter)) return undefined
   ...
   */
  async pollLogin(id: ProxiedProviderId, ticketId: string): Promise<LoginTicket | undefined> {
    const adapter = this.adapters.get(id)
    if (adapter === undefined || !canLogin(adapter)) return undefined
   ...
   */
  async logout(id: ProxiedProviderId): Promise<boolean> {
    const adapter = this.adapters.get(id)
    if (adapter === undefined || !canLogout(adapter)) return false
